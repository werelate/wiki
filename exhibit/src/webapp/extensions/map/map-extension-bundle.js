...
if(typeof f=="function"){view._settings.addOverlaysCallback=f;
}}var s=Exhibit.getAttribute(configElmt,"colorMarkerGenerator");
if(s!=null&&s.length>0){var f=eval(s);
if(typeof f=="function"){view._settings.colorMarkerGenerator=f;
}}var s=Exhibit.getAttribute(configElmt,"sizeMarkerGenerator");
if(s!=null&&s.length>0){var f=eval(s);
if(typeof f=="function"){view._settings.sizeMarkerGenerator=f;
}}var s=Exhibit.getAttribute(configElmt,"makeIcon");
if(s!=null&&s.length>0){var f=eval(s);
if(typeof f=="function"){view._settings.makeIcon=f;
...
if("addOverlaysCallback" in C){A._settings.addOverlaysCallback=C.addOverlaysCallback;
}if("colorMarkerGenerator" in C){A._settings.colorMarkerGenerator=C.colorMarkerGenerator;
}if("sizeMarkerGenerator" in C){A._settings.sizeMarkerGenerator=C.sizeMarkerGenerator;
}if("makeIcon" in C){A._settings.makeIcon=C.makeIcon;
}};
...
if(C.colorMarkerGenerator!=null){D.colorMarkerGenerator=C.colorMarkerGenerator;
}else{D.colorMarkerGenerator=function(F){var E=C.shape;
...
}if(C.sizeMarkerGenerator!=null){D.sizeMarkerGenerator=C.sizeMarkerGenerator;
}else{D.sizeMarkerGenerator=function(E){var F=C.shape;
...
Exhibit.MapView.prototype._reconstruct=function(){var H=this;
var X=this._uiContext.getCollection();
var a=this._uiContext.getDatabase();
var l=this._settings;
var m=this._accessors;
var K=X.countAllItems();
var N=X.countRestrictedItems();
var I=[];
this._map.clearOverlays();
this._dom.legendWidget.clear();
this._itemIDToMarker={};
if(N>0){var R=X.getRestrictedItems();
var c={};
var M=(this._accessors.getColorKey!=null);
var P=(this._accessors.getSizeKey!=null);
var C=(this._accessors.getIconKey!=null);
var n=(this._accessors.getIcon!=null);
R.visit(function(k){var i=[];
H._getLatlng(k,a,function(w){if(w!=null&&"lat" in w&&"lng" in w){i.push(w);
}});
if(i.length>0){var t=null;
if(M){t=new Exhibit.Set();
m.getColorKey(k,a,function(w){t.add(w);
});
}var q=null;
if(P){q=new Exhibit.Set();
m.getSizeKey(k,a,function(w){q.add(w);
});
}var s=null;
if(C){s=new Exhibit.Set();
m.getIconKey(k,a,function(w){s.add(w);
});
}for(var p=0;
p<i.length;
p++){var o=i[p];
...
if(u in c){var r=c[u];
r.items.push(k);
if(M){r.colorKeys.addSet(t);
}if(P){r.sizeKeys.addSet(q);
}if(C){r.iconKeys.addSet(s);
}}else{var r={latlng:o,items:[k]};
if(M){r.colorKeys=new Exhibit.Set();
r.colorKeys.addSet(t);
}if(P){r.sizeKeys=new Exhibit.Set();
r.sizeKeys.addSet(q);
}if(C){r.iconKeys=new Exhibit.Set();
r.iconKeys.addSet(s);
}c[u]=r;
}}}else{I.push(k);
}});
...
var V,Q=Infinity;
var B=function(p){var i=p.items.length;
if(!V){V=new GLatLngBounds();
}var q=H._settings.shape;
var k=H._settings.color;
if(M){k=H._colorCoder.translateSet(p.colorKeys,Y);
}var r=H._settings.iconSize;
if(P){r=H._sizeCoder.translateSet(p.sizeKeys,U);
}var s=null;
if(i==1){if(n){m.getIcon(p.items[0],a,function(w){s=w;
});
}}if(C){s=H._iconCoder.translateSet(p.iconKeys,G);
}var s;
if(H._settings.makeIcon!=null){s=H._settings.makeIcon(q,k,r,i==1?"":i.toString(),s,H._settings);
}else{s=Exhibit.MapView._makeIcon(q,k,r,i==1?"":i.toString(),s,H._settings);
}var u=new GLatLng(p.latlng.lat,p.latlng.lng);
var o=new GMarker(u,s);
if(Q>p.latlng.maxAutoZoom){Q=p.latlng.maxAutoZoom;
}V.extend(u);
GEvent.addListener(o,"click",function(){o.openInfoWindow(H._createInfoWindow(p.items));
if(H._selectListener!=null){H._selectListener.fire({itemIDs:p.items});
}});
H._map.addOverlay(o);
for(var t=0;
t<p.items.length;
t++){H._itemIDToMarker[p.items[t]]=o;
}};
for(var j in c){B(c[j]);
}if(H._settings.addOverlaysCallback!=null){R.visit(function(i){H._settings.addOverlaysCallback(H,i);
});
}if(M){var W=this._dom.legendWidget;
var h=this._colorCoder;
var S=Y.keys.toArray().sort();
if(l.colorLegendLabel!==null){W.addLegendLabel(l.colorLegendLabel,"color");
...
var T=h.translate(O);
W.addEntry(T,O);
}}if(Y.others){W.addEntry(h.getOthersColor(),h.getOthersLabel());
}if(Y.mixed){W.addEntry(h.getMixedColor(),h.getMixedLabel());
}if(Y.missing){W.addEntry(h.getMissingColor(),h.getMissingLabel());
}}if(P){var W=this._dom.legendWidget;
var D=this._sizeCoder;
var S=U.keys.toArray().sort();
if(l.sizeLegendLabel!==null){W.addLegendLabel(l.sizeLegendLabel,"size");
}if(D._gradientPoints!=null){var Z=D._gradientPoints;
var g=(Z[Z.length-1].value-Z[0].value)/5;
S=[];
for(var f=0;
f<6;
f++){S.push(Math.floor(Z[0].value+g*f));
}for(var e=0;
e<S.length;
e++){var O=S[e];
var F=D.translate(O);
...
var F=D.translate(O);
W.addEntry(F,O,"size");
}if(U.others){W.addEntry(D.getOthersSize(),D.getOthersLabel(),"size");
}if(U.mixed){W.addEntry(D.getMixedSize(),D.getMixedLabel(),"size");
}if(U.missing){W.addEntry(D.getMissingSize(),D.getMissingLabel(),"size");
}}}if(C){var W=this._dom.legendWidget;
var A=this._iconCoder;
var S=G.keys.toArray().sort();
if(l.iconLegendLabel!==null){W.addLegendLabel(l.iconLegendLabel,"icon");
}for(var e=0;
e<S.length;
e++){var O=S[e];
var J=A.translate(O);
W.addEntry(J,O,"icon");
}if(G.others){W.addEntry(A.getOthersIcon(),A.getOthersLabel(),"icon");
}if(G.mixed){W.addEntry(A.getMixedIcon(),A.getMixedLabel(),"icon");
}if(G.missing){W.addEntry(A.getMissingIcon(),A.getMissingLabel(),"icon");
}}if(V){var E=0.3;
var b=new GLatLng(V.getSouthWest().lat(),V.getSouthWest().lng()-E);
V.extend(b);
b=new GLatLng(V.getNorthEast().lat()+E,V.getNorthEast().lng()+E);
V.extend(b);
var d=Math.max(0,H._map.getBoundsZoomLevel(V));
if(l.maxAutoZoom==undefined){l.maxAutoZoom=Infinity;
}d=Math.min(d,Q,l.maxAutoZoom);
H._map.setZoom(d);
}if(V){H._map.setCenter(V.getCenter());
}}this._dom.setUnplottableMessage(N,I);
};
