...
function wfCite() {
	# Add messages
	global $wgMessageCache, $wgCiteMessages, $wgCite; // WERELATE - added $wgCite
	...
	class Cite {
      ...
      //WERELATE - add uncited ref
      function addUncitedRef($key, $str) {
         $this->mRefs[$key] = array(
            'text' => $str,
            'count' => -1, // not cited yet
            'number' => ++$this->mOutCnt
         );
      }

      //WERELATE - return whether there are references
      function hasReferences() {
		...
		function referencesFormat() {
         ...
         // WERELATE - clear state so we don't try to print references list again
         $this->clearState();
		...
		function referencesFormatEntry( $key, $val ) {
         ...
         // WERELATE - may have sources without references
         else if ( $val['count'] === -1 )
            return
               wfMsgForContentNoTrans(
                  'cite_references_link_zero',
                  $this->referencesKey( $key ),
                  $this->refKey( $key, $val['count'] ),
                  $val['text']
               );
