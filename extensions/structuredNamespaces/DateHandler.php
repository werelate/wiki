...
// Created Oct 2020 by Janet Bjorndahl
abstract class DateHandler {

  // Keep lists and logic in sync with EventDate.java (shared/org.werelate.util)
  // Test using DateHandlerWithTests (explorecode)
  ...
  public static function editDate($date, &$formatedDate, &$languageDate, $eventType=null, $reformatDetails=false) {    // reformatDetails added Mar 2021 by Janet Bjorndahl
    ...
    if ( isset($parsedDate['modifier'][1]) || isset($parsedDate['year'][1]) ) {   
      ...
      // if this is a discrete event, change From/to (misused) to Bet/and
      if ( in_array($eventType, self::$DISCRETE_EVENT) && $parsedDate['modifier'][1] == 'From' ) {  
...
//        }                      
   ...
   public static function getIsoDate($date) {
      $dateKey=self::getDateKey($date);  
      if (strlen($dateKey) >= 4) {
         $result = substr($dateKey, 0, 4);
         if (strlen($dateKey) >= 6) {
            $result .= '-'.substr($dateKey, 4, 2);
            if (strlen($dateKey) == 8) {
               $result .= '-'.substr($dateKey, 6, 2);
            }
         }
      }
      else {
  ...
  // Otherwise, it returns just the last year of the range. 
    ...
    if ( isset($parsedDate['year'][0]) ) {
      ...
      if ( isset($parsedDate['year'][1]) && $parsedDate['year'][0] == $parsedDate['year'][1] 
        ...
        if ( in_array($eventType, self::$DISCRETE_EVENT) && isset($parsedDate['modifier'][1]) && $parsedDate['modifier'][1] == 'From' ) {  
          $parsedDate['modifier'][1] = 'Bet';
          $parsedDate['modifier'][0] = 'and';
        }
  ...
  private static function parseDate($originalDate) {
    ...
    // Check for one or more embedded dates in dd-mmm-yyyy format (whether mmm is the abbreviation or full name of a month)
    ...
    if ( preg_match_all('#\d{1,2}-[A-Za-z]+-\d{3,4}#', $date, $fields, PREG_OFFSET_CAPTURE) > 0 ) {
      for ( $i=count($fields[0])-1; $i>=0 ; $i-- ) {
        $embeddedDate = str_replace('-', ' ', $fields[0][$i][0]);
        $date = substr($date,0,$fields[0][$i][1]) . $embeddedDate . substr($date,$fields[0][$i][1]+strlen($fields[0][$i][0]));
      }
    $fields = array();              // reinitialize fields (used again below)
    }
