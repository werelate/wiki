...
abstract class DateHandler {
  ...
  private static $MONTHS = array('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'dec');                                        
  ...
  public static function editDate($date, &$formatedDate, &$languageDate, $discreteEvent=false) {
    ...
    $date = mb_strtolower(trim(preg_replace('!\s+!', ' ', $date)));
    ...
    if ( ($begin = strpos($date, '(')) !== false && strrpos($date, ')') == strlen($date)-1 ) {
      if ( $begin > 0 ) {
        $text = substr($date, $begin);
        $date = substr($date, 0, $begin-1);
      }
      else {
        $formatedDate = $date;
        return true;
    ...
    for ($i=count($fields)-1; $i>=0; $i--) {
      $field = $fields[$i][1];
      if ( $field === '/' ) {
        if ( !isset($year[$dateIndex]) ) {                       // error if second part of the split year not already captured
        ...
        if ( $findSplitYear ) {                                  // if waiting for the first part of a split year, treat this field as the first part
          ...
          if ( $field === 'bce' || $field === 'bc' ) {
            ...
            if ( is_numeric($field) ) {
              ...
              if ( DateHandler::isDay($num) ) {                  // if this field could be a day, it could also be a year (unless month already captured)
                if ( isset($year[$dateIndex]) ) {                // if year is populated, this is the day; otherwise, this is the year
                  ...
                  if ( isset($month[$dateIndex]) ) {             // error if month already has a value (the number cannot have been meant as a year)
                    return 'Missing year';
                  }
                  else {
                    $year[$dateIndex] = "$num";             
              ...
              if ( $m = DateHandler::getMonthAbbrev($field) ) {
                ...
                if ( $q = DateHandler::getQualifier($field) ) {
                  if ( isset($qualifier[$dateIndex]) ) {         // error if qualifier already has a value
...
//        }                      
  ...
  private static function doubleDating($y) {
  ...
  private static function editSplitYear($firstPart, &$secondPart) {  // firstPart is an unknown field; secondPart qualifies as a valid year (numeric string without leading zeros)
    ...
    if ( !DateHandler::doubleDating($num) ) {
      return 'Split year not valid for this year';
    }
    if ( ($secondPart === '0' && substr($firstPart, 3) === '9') ) {
      $secondPart = $firstPart . '/' . substr($firstPart, 2, 1) + 1 . '0';
      return true;
    }
    $newYear = (substr($firstPart, 0, strlen($firstPart)-strlen($secondPart))) . $secondPart;
    if ( $newYear - $num == 1 ) {
      $secondPart = $firstPart . '/' . (substr($firstPart, 2) + 1);
      return true;
    }
