 ...
 * v 0.1.0 - 2007-03-03
 */
//WERELATE: convert mouseover/mouseout to mouseenter/mouseleave
(function($)
{
	var defaults = {
		onClick: function(){
	...
	$.fn.clickMenu = function(options) 
	{
		...
		var hideDIV = function(div, delay)
		{
			...
			if ( div.isVisible )
			{
				...
				{
					//remove events
					$(getAllChilds(getOneChild(div, 'UL'), 'LI')).unbind('mouseenter', liHoverIn).unbind('mouseleave', liHoverOut).unbind('click', settings.onClick);
					//hide it
					$(div).hide();
					div.isVisible = false;
					div.timer = null;
		...
		var showDIV = function(div, delay)
		{
			...
			if ( !div.isVisible )
			{
				div.timer = setTimeout(function()
				{
					...
					$(getAllChilds(getOneChild(div, 'UL'), 'LI')).mouseenter(liHoverIn).mouseleave(liHoverOut).click(settings.onClick);
					//positioning
					...
					$(div).show();
               // WERELATE: I don't like browser-specific things; we can do this always
...
//					}
		...
		var clean = function()
		{
			...
			//remove events
			$('ul.clickMenu>li li').unbind('mouseenter', liHoverIn).unbind('mouseleave', liHoverOut).unbind('click', settings.onClick);
